...
- 非显而易见的实现决策应通过就近注释说明原因，包括为何采用或放弃某方案、关键逻辑存在的必要性等，避免后续误删误改。
...
- 导入时尽量不用 `as` 重命名；优先使用明确的导出名或本地组合类型名，避免同一类型在不同文件中换名造成迷惑。
...
- pnpm monorepo + catalog 管理依赖：只有至少两个子包共用的依赖才放进 `pnpm-workspace.yaml` 的 `catalog`；单包依赖直接在子包 `package.json` 写版本号。
